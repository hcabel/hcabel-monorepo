...
export default function ProjectScrollBar(props: IProjectScrollBarProps)
{
	...
	function MoveScrollContent(start: number, end: number)
	{
		...
		setTimeout(() => {
			...
			scrollContent.style.transition = `top ${TransitionDuration / 1000}s ease-in-out`;
	...
	function changePage(projectIndex: number, direction: 0 | -1)
	{
		...
		const backgroundDivs = document.querySelectorAll<HTMLDivElement>('#BackgroundDivs > div');
		backgroundDivs.forEach((div) => {
			div.style.opacity = (div.id === `BackgroundDiv_${projectIndex + direction}` ? '1' : '0');
		});

		// once the animation is done, change the page
		setTimeout(() => {
			const previousProject = ProjectsInfos[projectIndex + direction];
			router.push(`/${locale}${previousProject.url}${direction === 0 ? '#bottom' : '#top'}`);
		}, TransitionDuration);
	}
	...
	useEffect(() => {
		...
		const currentprojectIndex = ProjectsInfos.findIndex((project) => pathname === `/${locale}${project.url}`);
		const currentBackgroundDiv = document.getElementById(`BackgroundDiv_${currentprojectIndex}`);
		...
		<div style={{ scrollBehavior: 'auto' }}>
			<div id="BackgroundDivs">
				{ProjectsInfos.map((project, i) =>
					<div key={i} id={`BackgroundDiv_${i}`} className={`${Style.Background} ${project.background}`} />
				)}
			</div>
			<div id="ScrollContent" className={Style.ScrollContent} style={{ top: "100vh" }}>
				{props.children}
			</div>
			<div id="InvisibleDivs">
				{ProjectsInfos.map((project, i) =>
					<div id={`InvisibleDiv_${i}`} key={i} style={{ height: `calc(100vh + ${project.height})` }} />
				)}
			</div>
		</div>
	);
