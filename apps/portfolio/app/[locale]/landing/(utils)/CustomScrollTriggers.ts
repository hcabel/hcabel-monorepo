...
function createMarkers(type: string, id: string, matchWidthEl: HTMLElement, pos: number, isFlip: boolean = false)
{
	const markersDefault = {startColor: "green", endColor: "red", indent: 0, fontSize: "16px", fontWeight:"normal"};
	...
	let css = "border-color:" + color + ";font-size:" + markersDefault.fontSize + ";color:" + color + ";font-weight:" + markersDefault.fontWeight + ";pointer-events:none;white-space:nowrap;font-family:sans-serif,Arial;z-index:1000;padding:4px 8px;border-width:0;border-style:solid;";
	...
	div.setAttribute("class", "gsap-marker-" + type + (id ? " marker-" + id : ""));
...
class Triggers {
	...
	get Progress(): number { return Math.max(0, Math.min(1, this._RealProgress)); }
	...
	constructor(args: TriggerArgs)
	{
		...
		this.start = args.trigger.offsetTop + (start[1] === "top" ? 0 : args.trigger.offsetHeight);
		this.startTrigger = 0 + (start[0] === "top" ? 0 : window.innerHeight);
		this.end = args.trigger.offsetTop + (end[1] === "top" ? 0 : args.trigger.offsetHeight);
		...
		if (this.markers) {
			...
			this._Markers = [
				...
				createMarkers("scroller-start", this.id, args.trigger, this.startTrigger),
				createMarkers("end", this.id, args.trigger, this.end, true),
				createMarkers("scroller-end", this.id, args.trigger, this.endTrigger)
			];
		}
		...
		// Observe the trigger
		this._ElementObserver = new IntersectionObserver((entries) => {
			if (this._Enable) {
				entries.forEach((entry) => {
					if (entry.isIntersecting) {
						// Reset the progress before doing anything (otherwise it may trigger events when it shouldn't)
						this._RealProgress = this.CalculateCurrentProgress();
	...
	private documentScroll()
	{
		...
		else if (this._RealProgress >= 0 && this._RealProgress <= 1 && progress < 0) {
			this.onLeave();
		}
		else if (this._RealProgress >= 0 && this._RealProgress <= 1 && progress > 1) {
			this.onLeaveBack();
		}
...
export class CustomScrollTriggers {
	...
	get Triggers(): Triggers[] { return this._Triggers; }
	...
	public kill(id: string)
	{
		...
		if (trigger) {
			trigger.cleanUp();
			this._Triggers = this._Triggers.filter((trigger) => trigger.id !== id);
		}
	}
